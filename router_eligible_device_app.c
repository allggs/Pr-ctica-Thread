...
const coapUriPath_t gAPP_TEAM1_URI_PATH = {SizeOfString(APP_TEAM1_URI_PATH), (uint8_t *)APP_TEAM1_URI_PATH};
#if LARGE_NETWORK
...
static tmrTimerID_t gTeam1Timer = gTmrInvalidTimerID_c;
static uint32_t gTeam1Contador = 0;
...
)
{
	...
	// Es CON o NON?
	const char* msgType = (pSession->msgType == gCoapConfirmable_c) ? "CON" : "NON";
	shell_printf("\r\n'%s' instruction received from %s\r\n", msgType, addrStr);

	// respuesta solo si es CON
	if(pSession->msgType == gCoapConfirmable_c) {
		uint8_t payload[4];
		shell_printf((char*)payload, sizeof(payload), "%lu", gTeam1Contador);
		COAP_Send(pSession, gCoapMsgTypeAckSuccessContent_c, payload, strlen((char*)payload));
	}
}
static void APP_Team1TimerCb(void *param)
{
	if(gTeam1Contador < 200) {
		gTeam1Contador++;
	} else {
		gTeam1Contador = 0; // reiniciar timer
	}
}
/*==================================================================================================
Public functions
...
#endif
	gTeam1Timer = TMR_AllocateTimer();
	if(gTeam1Timer != gTmrInvalidTimerID_c) {
		TMR_StartIntervalTimer(gTeam1Timer, 1000, APP_Team1TimerCb, NULL);
	}
...
)
{
	coapRegCbParams_t cbParams[] =  {{APP_CoapLedCb,  (coapUriPath_t *)&gAPP_LED_URI_PATH},
			...
			{APP_CoapTeam1Cb, (coapUriPath_t *)&gAPP_TEAM1_URI_PATH},//Agregamos el coap Parte1
